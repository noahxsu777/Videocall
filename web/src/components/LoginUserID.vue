...
import { TUIButton, TUIToast, TOAST_TYPE } from '@tencentcloud/uikit-base-component-vue3';
...
export interface LoginProps {
  SDKAppID: number;
  className?: string;
  onLoginCallback?: (res: any) => void;
  generatorUserSig?: (userID: string) => string;
}

const props = defineProps<LoginProps>();

const { SDKAppID = 0, onLoginCallback, generatorUserSig, ...otherConfig } = props;
...
if (currentUserInfo) {
  try {
    const storedInfo = JSON.parse(currentUserInfo);
    if (storedInfo.SDKAppID === SDKAppID) {
      userInfo.value.userID = storedInfo.userID;
      isLogin.value = true;
    }
  } catch (e) {
...
const login = (loginInfo: any) => {
  ...
  onLoginCallback && onLoginCallback(loginInfo);
  isLogin.value = true;
};
...
const submitForm = (): void => {
  if (!SDKAppID) {
    TUIToast({
      ...
      message: 'Falta configurar el SDKAppID en src/config/basic-info-config.js',
    });
    return;
  }
  ...
  if (!generatorUserSig) {
    TUIToast({
      type: TOAST_TYPE.ERROR,
      message: 'Falta la función para generar el userSig',
    });
    return;
  }
  ...
  handleLogin({
    ...
    userSig: generatorUserSig(userInfo.value.userID) || '',
  });
};
</script>
